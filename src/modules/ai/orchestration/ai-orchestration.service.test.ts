...
import { defaultAIPipelines } from "../pipelines/pipeline-catalog.js";
import { AIPipelineRegistry } from "../pipelines/pipeline-registry.js";
import { defaultPromptTemplates } from "../prompts/prompt-catalog.js";
import { PromptRegistry } from "../prompts/prompt-registry.js";
import type {
  PipelineExecutionScheduler,
  PipelineExecutionStore,
} from "../execution/pipeline-execution-store.js";
...
test("AI orchestration queues dataset interpretation with pipeline metadata", async () => {
  ...
  assert.equal(
    (queuedPayload as { ai?: { pipelineKey?: string } } | null)?.ai?.pipelineKey,
    "interpret_dataset",
  );
});

import {
  AIArtifactMongoModel,
  type AIArtifactMongoHydratedDocument,
} from "./ai-artifact.model.js";
import type { DatabaseSession } from "../../../shared/database/database-client.js";
import { createDocumentId } from "../../../shared/database/document-id.js";
import { AppError } from "../../../shared/errors/app-error.js";
import type { ResultRepository } from "./result.repository.js";
import type {
  AIArtifactCreateInput,
  AIArtifactPersistenceRecord,
  AIArtifactUpdateInput,
} from "../persistence/ai-persistence.types.js";

function toPlainArtifact(
  document: AIArtifactMongoHydratedDocument | null,
): AIArtifactPersistenceRecord | null {
  if (!document) {
    return null;
  }

  return {
    id: document._id.toString(),
    organizationId: document.organizationId,
    projectId: document.projectId,
    activityId: document.activityId ?? null,
    uploadMetadataId: document.uploadMetadataId ?? null,
    processingJobId: document.processingJobId ?? null,
    createdById: document.createdById,
    resultType: document.resultType,
    status: document.status,
    payload:
      document.payload && typeof document.payload === "object"
        ? (document.payload as Record<string, unknown>)
        : null,
    createdAt: document.createdAt,
    updatedAt: document.updatedAt,
  };
}

export class MongoAIArtifactRepository implements ResultRepository {
  async create(
    input: AIArtifactCreateInput,
    _session: DatabaseSession,
  ): Promise<AIArtifactPersistenceRecord> {
    const document = await AIArtifactMongoModel.create({
      _id: createDocumentId(),
      ...input,
      status: "pending",
    });

    return toPlainArtifact(document) as AIArtifactPersistenceRecord;
  }

  async findById(
    artifactId: string,
    _session: DatabaseSession,
  ): Promise<AIArtifactPersistenceRecord | null> {
    const document = await AIArtifactMongoModel.findById(artifactId).exec();
    return toPlainArtifact(document);
  }

  async listByActivity(
    activityId: string,
    _session: DatabaseSession,
  ): Promise<AIArtifactPersistenceRecord[]> {
    const documents = await AIArtifactMongoModel.find({ activityId })
      .sort({ createdAt: -1 })
      .exec();

    return documents
      .map((document) => toPlainArtifact(document))
      .filter((document): document is AIArtifactPersistenceRecord => Boolean(document));
  }

  async listRecentByProject(
    projectId: string,
    limit: number,
    _session: DatabaseSession,
  ): Promise<Array<Pick<
    AIArtifactPersistenceRecord,
    "id" | "activityId" | "status" | "createdAt"
  >>> {
    const documents = await AIArtifactMongoModel.find({ projectId })
      .sort({ createdAt: -1 })
      .limit(limit)
      .select({
        _id: 1,
        activityId: 1,
        status: 1,
        createdAt: 1,
      })
      .exec();

    return documents.map((document) => ({
      id: document._id.toString(),
      activityId: document.activityId ?? null,
      status: document.status,
      createdAt: document.createdAt,
    }));
  }

  async countByActivityIds(
    activityIds: string[],
    _session: DatabaseSession,
  ): Promise<Record<string, number>> {
    if (activityIds.length === 0) {
      return {};
    }

    const groupedDocuments = await AIArtifactMongoModel.aggregate<{
      _id: string | null;
      count: number;
    }>([
      {
        $match: {
          activityId: {
            $in: activityIds,
          },
        },
      },
      {
        $group: {
          _id: "$activityId",
          count: { $sum: 1 },
        },
      },
    ]).exec();

    return Object.fromEntries(
      groupedDocuments
        .filter(
          (document): document is { _id: string; count: number } =>
            typeof document._id === "string",
        )
        .map((document) => [document._id, document.count]),
    );
  }

  async countByProjectStatuses(
    projectId: string,
    statuses: AIArtifactPersistenceRecord["status"][],
    _session: DatabaseSession,
  ): Promise<number> {
    return AIArtifactMongoModel.countDocuments({
      projectId,
      status: {
        $in: statuses,
      },
    }).exec();
  }

  async update(
    artifactId: string,
    input: AIArtifactUpdateInput,
    _session: DatabaseSession,
  ): Promise<AIArtifactPersistenceRecord> {
    const document = await AIArtifactMongoModel.findByIdAndUpdate(
      artifactId,
      {
        $set: input,
      },
      {
        new: true,
      },
    ).exec();

    const record = toPlainArtifact(document);

    if (!record) {
      throw new AppError("Result record not found.", 404, "result_record_not_found");
    }

    return record;
  }
}
